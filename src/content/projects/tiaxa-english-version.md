---
title: "Tiaxa: Water Data Management"
slug: tiaxa-english-version
area: motion
date: 2019-09-29
summary: Visualización de procesos y optimización de recursos hídricos mediante
  tecnología IoT para la industria minera.
detailLead: Motion graphics desarrollado para Tiaxa, enfocado en su solución
  Water Data Management. El video explica cómo la integración de tecnología IoT,
  analítica de datos y aplicaciones móviles permite optimizar el uso del recurso
  hídrico en faenas mineras de difícil acceso.
cover: /media/images/projects/vlcsnap-2026-04-13-16h34m23s783.png
tags: |-
  Motion Graphics
  Minería
  Tiaxa
  IoT
  Data Management
  Animación 2D
  Sustentabilidad
featured: true
draft: false
videoUrl: https://youtu.be/2_YFZvnaoZk
---
Este video técnico-explicativo aborda la problemática del control hídrico en la minería, destacando soluciones que reducen hasta en un **85% los tiempos de respuesta** ante desviaciones de caudales y eliminan el error humano en lecturas manuales.

*(Nota: Esta es la versión del video con textos en inglés).*

![image.png](/media/images/projects/image.png)

### Créditos y Colaboración

Este proyecto fue realizado a través de **MT Audiovisual (Mario Toro)**, trabajando en conjunto con un equipo especializado:

- **Animación y Motion Graphics:** Erick Guerrero / [Nifty.cl](http://Nifty.cl)
- **Diseño y Dirección de Arte:** [Katherine Sánchez](https://portrait.so/kathonejo) / [kathonejo@gmail.com](mailto:kathonejo@gmail.com)

---

### *Tiaxa: Water Data Management (English Version)*

*Motion graphics developed for **Tiaxa**, focusing on their "Water Data Management" solution. The video illustrates how the integration of IoT technology, data analytics, and mobile applications optimizes water resource management in remote mining operations. It highlights key benefits such as reduced response times and the elimination of human error in data collection.*

*Credits & Collaboration* *This project was produced through **MT Audiovisual (Mario Toro)**, in collaboration with a specialized team:*

- ***Animation & Motion Graphics:** Erick Guerrero / [Nifty.cl](http://Nifty.cl)*
- ***Graphic Design & Art Direction:** [Katherine Sánchez](https://portrait.so/kathonejo) / [kathonejo@gmail.com](mailto:kathonejo@gmail.com)*

